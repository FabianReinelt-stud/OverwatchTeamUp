...
from heroes.models import TeamComposition, Hero
...
class TeamCompositionDatabaseAdapter(TeamCompositionPort):
    ...
    def get_all(self) -> list[TeamCompositionEntity]:
        return [self._to_entity(tc) for tc in TeamComposition.objects.all()]

    def get_by_id(self, team_id: int) -> TeamCompositionEntity:
        return self._to_entity(TeamComposition.objects.get(pk=team_id))
    ...
    def update(self, team_id: int, team: TeamCompositionEntity) -> TeamCompositionEntity:
        team_comp = TeamComposition.objects.get(pk=team_id)
    ...
    def delete(self, team_id: int) -> None:
        TeamComposition.objects.get(pk=team_id).delete()
